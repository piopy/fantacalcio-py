...
from src.providers import fetch_understat, load_fp, fetch_provider_stats
...
def main():
    ...
    cols = {"Nome": "Calciatore", "Squadra": f"Squadra Attuale ({args.anno}-{args.anno + 1})", "Ruolo": "Ruolo", "Punteggio_Asta_100": "Punteggio Asta (/100)", cred_col: cred_col, "Hidden_Gem": "Hidden Gem?", "Motivo_Hidden_Gem": "Motivo", "Alternative_Consigliate": "Alternative Affini", f"Fantamedia anno {args.anno - 1}-{args.anno}": "Fantamedia Prev", f"Fantamedia anno {args.anno}-{args.anno + 1}": "Fantamedia Live", "up_team": f"Squadra {args.anno - 1}-{args.anno}", "up_games": f"Pres {yy_prev}", "up_goals": f"Gol {yy_prev}", "up_xG": f"xG {yy_prev}", "up_assists": f"Assist {yy_prev}", "up_xA": f"xA {yy_prev}", "uc_games": f"Pres {yy_curr}", "uc_goals": f"Gol {yy_curr}", "uc_xG": f"xG {yy_curr}", "uc_assists": f"Assist {yy_curr}", "uc_xA": f"xA {yy_curr}", "Skills": "Skills", "Infortunato": "Infortunato", "Trend": "Trend", "Consigliato prossima giornata": "Consigliato", "Buon investimento": "Buon invest.", "Resistenza infortuni": "Resist. infort.", "Gol previsti": "Gol prev.", "Assist previsti": "Assist prev.", "ext_fanta": "Indice Esterno", "ext_tit": "Titolarità", "ext_cont": "Continuità", "ext_mv": "MV Fonte Est."}
