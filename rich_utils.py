...
class StatusDisplay:
    ...
    def show_banner(self):
        ...
        banner = Panel(
            ...
            + Text(
                "\nAdvanced Fantasy Football Analysis Tool",
                style="cyan",
                justify="center",
            ),
            title="Welcome",
    ...
    def show_top_players_table(
        self, df, source_name: str, top_n: int = 20, convenience_col: str = None
    ):
        ...
        table.add_column(
            "💎 Convenience", justify="right", style="bold green", width=12
        )
        ...
        if source_name == "FPEDIA":
            if "Punteggio" in df.columns:
                table.add_column(
                    "📊 Score", justify="right", style="bright_blue", width=8
                )
            if "Presenze campionato corrente" in df.columns:
                table.add_column(
                    "🎯 Appearances", justify="right", style="cyan", width=12
                )
        else:  # FSTATS
            if "fanta_avg" in df.columns:
                table.add_column(
                    "📈 Fanta Avg", justify="right", style="bright_blue", width=10
                )
            if "presences" in df.columns:
                table.add_column(
                    "🎯 Presences", justify="right", style="cyan", width=10
                )
        ...
        for idx, (_, row) in enumerate(df_top.iterrows(), 1):
            ...
            if source_name == "FPEDIA":
                ...
                if "fanta_avg" in df.columns:
                    ...
                    row_data.append(
                        f"{fanta_avg:.2f}" if pd.notna(fanta_avg) else "N/A"
                    )
                if "presences" in df.columns:
                    presences = row.get("presences", 0)
                    row_data.append(
                        str(int(presences)) if pd.notna(presences) else "N/A"
                    )
...
def simulate_progress_task(
    progress: Progress, task_id: TaskID, total_steps: int, description: str
):
    """Simulate a progress task for demonstration"""
    for i in range(total_steps):
        time.sleep(0.1)  # Simulate work
        progress.update(
            task_id, advance=1, description=f"{description} (step {i+1}/{total_steps})"
        )
