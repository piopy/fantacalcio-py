...
## Env e cache
- `.env` richiesto: `FSTATS_MAIL` + `FSTATS_PASSWORD` (nomi tecnici, autenticano il provider esterno).
- Cache in `data/`: `_giocatori.csv`, `giocatori_urls.txt`, `stats_<anno>.json`, `provider_ext.json`. Run riusa cache se presente.
